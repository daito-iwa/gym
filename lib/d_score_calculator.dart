      ...
      if (connectionGroup.length > 1) {
        for (int i = 0; i < connectionGroup.length - 1; i++) {
          ...
          if (skill1.group == 2 && skill2.group == 2) {
            print('🔍 鉄棒手放し技連続: ${skill1.name}(難度値:${v1}, レター:${skill1.valueLetter}) + ${skill2.name}(難度値:${v2}, レター:${skill2.valueLetter})');
            print('  条件チェック: v1(${v1}) >= 0.4? ${v1 >= 0.4}, v2(${v2}) >= 0.5? ${v2 >= 0.5}');
            print('  逆方向チェック: v1(${v1}) >= 0.5? ${v1 >= 0.5}, v2(${v2}) >= 0.4? ${v2 >= 0.4}');
            
            // D難度以上 + E難度以上 = 0.20点（双方向）
            if ((v1 >= 0.4 && v2 >= 0.5) || (v1 >= 0.5 && v2 >= 0.4)) {
              bonusForThisPair = 0.2;
              print('  → 0.2点: D以上+E以上の条件にマッチ');
            }
            // D難度 + D難度 = 0.10点
            else if (v1 >= 0.4 && v2 >= 0.4) {
              bonusForThisPair = 0.1;
              print('  → 0.1点: D+Dの条件にマッチ');
            }
            // C難度 + D難度以上 = 0.10点（双方向）
            else if ((v1 == 0.3 && v2 >= 0.4) || (v1 >= 0.4 && v2 == 0.3)) {
              bonusForThisPair = 0.1;
              print('  → 0.1点: C+D以上の条件にマッチ');
            }
            else {
              print('  → 0.0点: 条件にマッチせず');
            }
